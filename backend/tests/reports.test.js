...
test.before(async () => {
  ...
  const admin = await User.create({ name: 'Report Admin', email: 'report.admin@test.local', password, role: 'ADMIN' });
  ...
  adminToken = jwt.sign({ id: admin._id, role: admin.role }, secret);
...
test('reports enforce HR authorization and return real attendance and leave data', async () => {
  ...
  assert.equal((await request('/api/reports/attendance', adminToken)).response.status, 200);
});
...
test('reports apply employee, department, date, status, and empty filters', async () => {
  const paths = [
    ...
    `/api/reports/attendance?employee=${employee._id}&department=${department._id}`,
  ...
  assert.equal(results[4].body.data.attendance.length, 1);
  assert.equal(results[5].body.data.attendance.length, 0);
  assert.equal(results[5].body.data.leaves.length, 0);
});
