...
const buildFilter = async (query) => {
  ...
  if (query.department) {
    ...
    const departmentEmployees = users.map((user) => user._id);
    filter.employee = query.employee ? { $in: departmentEmployees.filter((id) => id.toString() === query.employee) } : { $in: departmentEmployees };
  }
...
const getAttendanceReport = async (req, res, next) => {
  try {
    ...
    const [attendance, leaves] = await Promise.all([
      ...
      }).populate({ path: 'employee', select: '-password', populate: { path: 'department' } }).sort({ startDate: -1 }).limit(1000),
    ]);
